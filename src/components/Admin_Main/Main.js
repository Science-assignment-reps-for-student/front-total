...
import { 
    ...
    teamHomeworkURL, 
    getUserInfoURL, 
    experimentHomeworkURL, 
    allFileDownloadURL, 
    getFileCodeURL, 
    excelFileDownloadURL 
...
const AdminMain = ({ state, actions, history, stomp }) => {
    ...
    },[actions, history, refreshToken])

    const getFileCodePromise = useCallback(
        (contentId) => 
    ...
    },[downloadFile, fileErrorCheck, fileHeader, getFileCodePromise])

    const excelFileUpdate = useCallback(
        (contentId) => new Promise((resolve,reject) => {
    ...
    },[downloadFile, excelFileUpdate, fileHeader, fileErrorCheck, getFileCodePromise])
    ...
    const makeContent = useCallback(
        ...
        return buffer;
    },[getExcelFile, zipFileDownload])

    const getExperimentHomework = useCallback(
    (url,header,checked,data,content) => {
    ...
    },[actions, history, contentChange, dataChange, loadedChange, makeContent, refreshToken])

    const getTeamHomework = useCallback(
    (url,header,checked,data,content) => {
        axios.get(url,header)
        .then((e)=> {
            const homeworkData = e.data;
    ...
    },[actions, history, makeContent, refreshToken, getExperimentHomework])

    const getPersonalHomework = useCallback(
    (url,header,checked,data,content) => {
        axios.get(url,header)
        .then((e)=> {
            const homeworkData = e.data;
    ...
    },[actions, history, getTeamHomework, makeContent, refreshToken])

    const sortContentList = useCallback(
    (contentList) => {
    ...
    const getArrowHomework = useCallback(
    (homeworkType,content) => {
        ...
        return result;
    },[count, sortContentList])

    const isHomeworkTypeAllTrue = useCallback(
    (homeworkType) => {
    ...
    },[count, homeworkType, isHomeworkTypeAllTrue])
